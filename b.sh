...
sudo apt-get -qq -y install texlive-latex-extra texlive-latex-recommended texlive-fonts-recommended pandoc pandoc-citeproc r-base-core qpdf texlive-latex-base texlive-base  xzdec texinfo ccache tex4ht texlive-fonts-extra r-cran-rcurl
...
R -q -e "options(repos=structure(c(CRAN = 'http://cran.rstudio.com'))); for (pkg in c('inline', 'StanHeaders', 'loo', 'Rcpp', 'RcppEigen', 'ggplot2',  'gridExtra', 'RUnit', 'BH', 'RInside', 'coda')) if (!require(pkg, character.only = TRUE))  install.packages(pkg, dep = TRUE); sessionInfo()"
